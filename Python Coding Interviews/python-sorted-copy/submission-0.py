...
def sort_words(words: List[str]) -> List[str]:
    dup_sorted_words = sorted(words)
    return dup_sorted_words


def sort_numbers(numbers: List[int]) -> List[int]:
    dup_sorted_numbers = sorted(numbers, key=lambda number: abs(number), reverse=True)
    return dup_sorted_numbers
